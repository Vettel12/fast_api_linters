...
class RecipeShort(BaseModel):
    ...
    cooking_time: int = Field(..., description="Время приготовления в минутах", examples=[10])
    views_count: int = Field(..., description="Количество просмотров рецепта", examples=[100])

    model_config = ConfigDict(from_attributes=True) # Позволяет Pydantic работать с объектами SQLAlchemy (Row)
